<?php 
    if($_SESSION['status'] == "teacher") {
        header("Location: index.php?action=home_add_test.php");
        exit(); 
    }
    else { 
?>
...
<html>
    ...
    <?php
        while($data = $request->fetch()) {
            if($data['status'] !== 'null') {
                $test = showDate($data['id_test']);
                if($test == true) {
                    $test1 = showTime($data['id_test']);
                    $end = ending($data['id_test']);
                    if($test1 == true) {
                        modifProgress('progress', $data['id_test']);
            ...
            $message = printForStudent($data['status'], $data['heure_start'] , $data['heure_fin'], $data['date']);
            ?>
        <div class="basic_box box red_section">
            <h3 class="heading_red"> test de 
                ...
                if($message == "set") {
                    ...
                    <label class="profil_titre" for="Jour">L'évaluation se déroulera le : </label>
                        ...
                        <label class="profil_titre" for="Mois"> Heure : <?php echo $data['heure_start']; ?></label>
                    </div>
                    <?php
                }
                else {
                ...
                ?>
            <form action="index.php?action=test.php" class="form_mdp" method="POST">
                <?php 
                if($data['status'] == "progress") {
                    echo "<input type='submit' class='btn btn--green btn_section' value='Accèder' id='btn-eval' name='afficher'>
                    <input type='hidden' name='status' value='$data[status]'>
                    <input type='hidden' name='id_eval' value='$data[id_test]'>
